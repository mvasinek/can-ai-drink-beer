...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Use get_next_task to find work, then mark_task_done when finished."
    ),
)


def _run_with_service(action):
...
def get_next_task() -> dict:
    ...
    return _run_with_service(get_next_task_result)


@mcp.tool()
def mark_task_done(task_id: int) -> dict:
    """Mark a task as done and set its completion timestamp."""
    return _run_with_service(lambda service: mark_task_done_result(service, task_id))


@mcp.tool()
def get_task(task_id: int) -> dict:
    """Return complete information for a task by ID."""
    return _run_with_service(lambda service: get_task_result(service, task_id))


@mcp.tool()
def list_open_tasks() -> dict:
    """List all tasks with status open."""
    return _run_with_service(list_open_tasks_result)


@mcp.resource("tasks://open")
def open_tasks_resource() -> str:
    """Expose all open tasks as a JSON resource."""
    result = _run_with_service(get_open_tasks_resource)
